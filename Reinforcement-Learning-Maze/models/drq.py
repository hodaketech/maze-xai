...
def plot_training_summary(reward_log):
    episodes = list(range(len(reward_log)))
    steps = [entry['steps'] for entry in reward_log]
    total_rewards = [sum(entry['rewards']) for entry in reward_log]

    fig, ax = plt.subplots(2, 1, figsize=(12, 8))

    ax[0].plot(episodes, steps, color='blue', label='Steps per Episode')
    ...
    ax[1].plot(episodes, total_rewards, color='green', label='Total Reward')
